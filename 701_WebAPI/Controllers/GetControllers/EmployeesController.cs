...
namespace _701_WebAPI.Controllers.GetControllers
{
    ...
    public class EmployeesController : ControllerBase
    {
        ...
        public async Task<ActionResult> GetTimeSheet(string accountID, string date)
        {
            ...
            IRow headingRow = sheet.CreateRow(2);
            int columnCount = 5;
            ...
            foreach (TimeSheetRow item in rowList)
            {
                ...
                if (item.Hours != 0) row.CreateCell(3).SetCellValue(string.Format("{0:0.00}", item.Hours));
                else row.CreateCell(3).SetCellValue("");
                if (item.HoursOT != 0) row.CreateCell(4).SetCellValue(string.Format("{0:0.00}", item.HoursOT));
            ...
            rowTotal.CreateCell(3).SetCellValue(string.Format("Total: {0:0.00}", totalHours));
            rowTotal.CreateCell(4).SetCellValue(string.Format("Total: {0:0.00}", totalHoursOT));
